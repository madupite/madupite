...
class MDP:
    ...
    def valueIteration(self, V0, maxIter):
        ...
        for i in range(1, maxIter):
            ...
            policy[:] = np.argmax(costs, axis=1)
            V_old[:] = V
            V[:] = np.max(costs, axis=1)
    ...
    def extractGreedyPolicy2(self, V):
        ...
        maxCosts = np.full(self.numStates_, np.inf)
        ...
        for actionInd in range(self.numActions_):
            ...
            for stateInd in range(self.numStates_):
                if cost[stateInd] > maxCosts[stateInd]:
                    maxCosts[stateInd] = cost[stateInd]
