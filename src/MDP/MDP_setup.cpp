...
void MDP::generateCostMatrix(double (*g)(PetscInt, PetscInt)) {
    ...
    if (numStates_ == 0 || numActions_ == 0) {
        PetscThrow(comm_, 1, "Number of states and actions not set.");
    }

    // create stage cost matrix
    // ierr = MatCreateDense(comm_, localNumStates_, PETSC_DECIDE, numStates_, numActions_, PETSC_NULLPTR, &stageCostMatrix_); CHKERRQ(ierr);
    PetscCallThrow(MatCreate(comm_, &stageCostMatrix_));
