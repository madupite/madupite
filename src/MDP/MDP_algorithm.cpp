...
void MDP::iterativePolicyEvaluation(const Mat &jacobian, const Vec &stageCosts, Vec &V, KSPContext &ctx) {
    ...
    PetscCallThrow(KSPSetTolerances(ksp, 1e-40, ctx.threshold, PETSC_DEFAULT, ctx.maxIter)); // use L2 norm of residual to compare. This works since ||x||_2 >= ||x||_inf. Much faster than infinity norm.
