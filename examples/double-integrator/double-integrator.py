...
def main():
    ...
    g = md.createStageCostMatrix(
        numStates=NUM_STATES, numActions=NUM_ACTIONS, func=stage_cost
    )
    ...
    mdp.setOption(
        "-overwrite", "true"
    )  # overwrite existing files (so we don't need to adapt the filenames in the plotting script below)
