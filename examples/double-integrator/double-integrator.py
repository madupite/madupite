...
def step(s, a):
    x_t_i, xd_t_i = s2x(s)
    x_t_v, xd_t_v = x_vals[x_t_i], xd_vals[xd_t_i]
    a_t_v = a_vals[a]
...
def main():
    ...
    write_sparse_matrix_as_petsc_binary(P, f"data/di_P_{NUM_STATES}_{NUM_ACTIONS}.bin")
    write_sparse_matrix_as_petsc_binary(g, f"data/di_g_{NUM_STATES}_{NUM_ACTIONS}.bin")
    print(
        f"Matrices saved to data/di_P_{NUM_STATES}_{NUM_ACTIONS}.bin and data/di_g_{NUM_STATES}_{NUM_ACTIONS}.bin"
    )
    ...
    # plt.show()
    plt.savefig("double-integrator.png", dpi=300)
